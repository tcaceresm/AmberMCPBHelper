...
function ProcessMOL2() {
  ...
  # fix substructure
  awk '
    /@<TRIPOS>SUBSTRUCTURE/ { print; skip=1; next }  
    skip { skip--; next }                             
    { print }                                        
  ' ${OUTPUT} > ${OUTPUT}.tmp && mv ${OUTPUT}.tmp ${OUTPUT}
...
function CreateFinalLeap() {
  ...
  cat >> ${output_file} <<EOF
LG1 = loadmol2 LIGAND_renamed.mol2

EOF
  ExtractBonds ${input_file} ${output_file}

  sed -i 's/mol\.1/LG1.1/g' ${output_file}
  sed -i 's/mol\.2/LG1.1/g' ${output_file}

  cat >> ${output_file} <<EOF

set LG1 name "LG1"
saveoff LG1 LG1.lib
saveamberparm LG1 LG1.parm7 LG1.rst7
...
tleap -f final_leap.in

mkdir -p "relevant_files"

cp LG1.lib HYBRIDIZATION.data ${ORIGINAL_PDB_NAME}_mcpbpy.frcmod \
   LG1.pdb LG1.parm7 LG1.rst7 relevant_files
